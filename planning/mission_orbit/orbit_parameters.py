...
for key in orbit_d.keys():
    ...
    with PdfPages("%s.pdf" % key) as pdf:
        ...
        dt_starts = [orbit_d[key]["science_start"] + timedelta(days=delta_days*i)
                     for i in np.arange(np.floor((orbit_d[key]["science_end"] - orbit_d[key]["science_start"]).days/delta_days))]
        ...
        for mtp, dt_start in enumerate(dt_starts):
            ...
            title = "%s\nMTP%03i: %s - %s" % (orbit_d[key]["title"], mtp+1, datetime.strftime(dt_start,
                                              SHORT_DATETIME_FMT), datetime.strftime(dt_end, SHORT_DATETIME_FMT))
            fig.suptitle(title)
